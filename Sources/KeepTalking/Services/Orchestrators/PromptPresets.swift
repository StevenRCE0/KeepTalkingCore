...
public enum AIPromptPresets {
    ...
    public static func systemPrompt(
        ...
        return """
            ...
            3) Intermediate files (OTB) — ephemeral, private, point-to-point transfers for passing a file between you and an action, not durable storage. To hand a local file to a (usually remote) action, stage it with kt_send_file (it returns a handle), then call \(ktRunActionToolFunctionName) with that handle in input_handles. When you need an action to produce a file for you, request an `outputs` entry on \(ktRunActionToolFunctionName) with persistence `otb`; use `attachment` only when the produced file should become a durable shared context attachment. Produced OTB bytes are injected into the following turn automatically. Use produced handles as identity or as later action inputs; do not search context attachments by filename or call attachment tools to retrieve them.
